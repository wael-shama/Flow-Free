...
def backtrack(variables,allcolors):
    ...
    while(i >= 0 and i < len(variables)):
        ...
        if value == None:
            ...
            i += 1
            if i < len(variables):
                variables = MRV(variables,i,allcolors)
                variables[i]= resetVariable(variables[i], allcolors)        
    if i < 0 :
        return None
    else:
...
##    printvars(variables,length)
    ...
    t0 = time.time()
    variables = MRV(variables,0,allColors)
